...
public class DecisionMaking extends SimpleBehaviour{
	...
	public void action() {
		...
		for(i = 0; i < this.ListeTresor.size(); i++) { // On parcours les tresors
			...
			int j;
			for(j=0;j<this.ListeTresor.get(i).getRight().size();j++) { // On parcours les attributs des tr�sors
				int k;
				...
				case DIAMOND:
					...
					for(k = 0; k < this.ListeAgent.size(); k++) { // On parcours les agents
						...
						for(l=0;l<this.ListeAgent.get(k).getRight().size();l++) { // On parcours les attributs des agents  pour trouver celui qui corresponds
							...
							int diff = Math.abs(backpack - value); // On regarde si c'est le meilleur r�sultat possible
							if(diff < bestValue) { // On enregistre le r�sulta
								best_Collect = l;
								best_tresor = i;
								bestValue = diff;
							}
		...
		if(best_tresor >= 0 && best_Collect >= 0) { // On v�rifie qu'n couple a ete trouver
			...
			List<Couple<String,Integer>> Liste_Selection = new ArrayList<Couple<String,Integer>>(); // On y enregistrera la liste de agents selectionn�
				
			Integer k = 0; // servira a garder les index des agents selectionne
				
			while(localStrengh > 0 && localLock > 0 && k < this.ListeAgent.size()){ // tant que les capacit�s necessaire ne sont pas attente et que l'on a pas pris tous les agents pr�sent
				...
				for(j=0;j<this.ListeAgent.get(k).getRight().size();j++) { // On boucle sur les capacites de l'agent en cour de traitement
					...
					case "Strengh":
						if(localStrengh != 0 && this.ListeAgent.get(k).getRight().get(j).getRight() != 0) { // SI la force n'est pas nulle et la capacite differente de nulle 
							...
							if(localStrengh<0) {															// Si on descend en dessous de 0 On repositionne � 0
								localStrengh = 0;
							}
						}
						break;
